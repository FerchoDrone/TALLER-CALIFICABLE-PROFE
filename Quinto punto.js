...
function createRoom(){
    ...
    for (let i = 0; i < setup.rows; i++) {
        document.querySelector(".rows-container").innerHTML += `<div class="d-flex border border-primary justify-content-around p-3 ${setup.letters[i]}">
        <div class="border border-success p-3 seat">${setup.letters[i]}</div>`;
...
function seatInfo(letter, seatNumber){
 document.querySelector(".seat-info").innerHTML = `El número del vagón del tren seleccionado es: ${
    letter + seatNumber
 }`;
 document.querySelector(
    ".reserve-button"
 ).innerHTML = `<button type="button" class="btn btn-warning" onclick="reserve('${letter}',${seatNumber})">Reservar ${
 letter + seatNumber 
}</button>`;
...
function reserveTable() {
    ...
    for (let i = 0; i < DB.length; i++) {
        document.querySelector(".table tbody").innerHTML += `<tr><td>${DB[i].seatCode}</td><td>${DB[i].fullName}</td><td>${DB[i].cc}</td><td><button type="button" class="btn" onclick="deleteReservation('${DB[i].seatCode}')"><i class="bi bi-trash-fill"></i></button></td></tr>`
